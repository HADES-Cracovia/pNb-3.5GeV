...
int fitL1520andSigmaandDpp_smooth(void)
{
  ...
  TFile *fout=new TFile("fitL1520andSigmaandDpp_smooth_output.root","recreate");
  ...
  //hPPip_LDppKz->Scale(1./hPPip_LDppKz->Integral()*lum*LDppKz_eff/3*29.45/1000*TMath::Power(93,2/3)*arbitrary_scale);
  ...
  //D++ smoothing
  int ntimes=1;
  hPPimPipPim_LDppKz->Smooth(ntimes);
  hPPimPip_LDppKz->Smooth(ntimes);
  hPPimPim_LDppKz->Smooth(ntimes);
  hPPip_LDppKz->Smooth(ntimes);
	  ...
	  for(int l=1;l<=steps;l++)
	    {
	      ...
	      LPPip_sum->Add(hPPip_LDppKz,scaleDpp);
