...
void HParticlePool::loop(HIterator* dataIt)
{
   ...
   while (((ParticleCand = (HParticleCand *) dataIt->Next()) != 0))
   {
      ...
      //isRing = ParticleCand->getRingCorr() || ( (ParticleCand->getRichBTInd() != -1 ) );
      //isRing = ParticleCand->getRingCorr();
      //isRing = ( (ParticleCand->getRichBTInd() != -1 ) );
      ...
      if (isPositive && isRing) myEId = eLeptonPos;
         ...
         if ( simflag > 0 ) p = static_cast<HParticleCandSim*>( ParticleCand );
          ...
          if ( ParticleCand->isFlagBit(Particle::kIsUsed) == 1)
          //if ( ParticleCand->getChi2() < 500. && ParticleCand->getSystem()>=0 )
          {
           if ( simflag > 0 )
           {
...
//-----------------------------------------------------------------------------
void HParticlePool::fill(HIterator* dataIt, HIterator *geantIt, HIterator *startIt, HIterator *btIt) 
{
    ...
    HVertex& vertexcluster = gHades->getCurrentEvent()->getHeader()->getVertexCluster();

	eventData.set("eVertClust_x", vertexcluster.getX());
	eventData.set("eVertClust_y", vertexcluster.getY());
	eventData.set("eVertClust_z", vertexcluster.getZ());
	eventData.set("eVertClust_chi2", vertexcluster.getChi2());

    HVertex& vertexreco = gHades->getCurrentEvent()->getHeader()->getVertexReco();

	eventData.set("eVertReco_x", vertexreco.getX());
	eventData.set("eVertReco_y", vertexreco.getY());
	eventData.set("eVertReco_z", vertexreco.getZ());
	eventData.set("eVertReco_chi2", vertexreco.getChi2());
/*
    HParticleEvtInfo* evtInfo = HCategoryManager::getObject(evtInfo,catParticleEvtInfo,0,kTRUE);

    eventData.set("evtVertClust", evtInfo->isGoodEvent( Particle::kGoodVertexClust ) );
    eventData.set("evtVertCand", evtInfo->isGoodEvent( Particle::kGoodVertexCand ) );
    eventData.set("evtStart", evtInfo->isGoodEvent( Particle::kGoodSTART ) );
    eventData.set("evtPileupStart", evtInfo->isGoodEvent( Particle::kNoPileUpSTART ) );
    eventData.set("evtPileupMeta", evtInfo->isGoodEvent( Particle::kNoPileUpMETA ) );
    eventData.set("evtPileupMDC", evtInfo->isGoodEvent( Particle::kNoPileUpMDC ) );
    eventData.set("evtFlashMDC", evtInfo->isGoodEvent( Particle::kNoFlashMDC ) );
    eventData.set("evtMDCMult", evtInfo->isGoodEvent( Particle::kGoodMDCMult ) );
    eventData.set("evtLepMult", evtInfo->isGoodEvent( Particle::kGoodLepMult ) );
    eventData.set("evtGoodTrig", evtInfo->isGoodEvent( Particle::kGoodTRIGGER ) );
    eventData.set("evtGood", evtInfo->isGoodEvent( Particle::kGoodVertexClust|Particle::kGoodSTART|Particle::kNoPileUpSTART|Particle::kGoodTRIGGER ) );
*/
...
// -------------------------------------------------------------
    ...
    if ( startflag > 0 ) 
    {
       ...
       eventData.set("start_flag", -100. );
       //eventData.set("start_corrflag", -100. );
       eventData.set("start_module", -100. );
       eventData.set("start_strip", -100. );
       //eventData.set("start_multipliticy", -100. );
       eventData.set("start_time", -100. );
       //eventData.set("start_width", -100. );
       //eventData.set("start_resolution", -100. );
       //eventData.set("start_track", -100. );
       eventData.set("start_mult", -100. );
       eventData.set("start_counter", -100. );
       //eventData.set("start_time_2nd", -100. );
       //eventData.set("start_cluster_size_1st", -100. );
       //eventData.set("start_cluster_size_2nd", -100. );
       //eventData.set("start_cluster_size_max", -100. );
    }


    if ( startflag > 0 && startIt != 0 )
    {
       startIt->Reset();
       while (((StartCand = (HStartHit*) startIt->Next()) != 0))
       {
          ++smult;
       ...
       while (((StartCand = (HStartHit*) startIt->Next()) != 0))
       {
          eventData.set("start_flag", StartCand->getFlag() );
          //eventData.set("start_corrflag", StartCand->getCorrFlag() );
          eventData.set("start_module", StartCand->getModule() );
          eventData.set("start_strip", StartCand->getStrip() );
          //eventData.set("start_multipliticy", StartCand->getMultiplicity() );
          eventData.set("start_time", StartCand->getTime() );
          //eventData.set("start_width", StartCand->getWidth() );
          //eventData.set("start_resolution", StartCand->getResolution() );
          //eventData.set("start_track", StartCand->getTrack() );
          eventData.set("start_mult", smult );
          eventData.set("start_counter", scounter );
          //eventData.set("start_time_2nd", StartCand->getSecondTime() );
          //eventData.set("start_cluster_size_1st", StartCand->getFirstClusterSize() );
          //eventData.set("start_cluster_size_2nd", StartCand->getSecondClusterSize() );
          //eventData.set("start_cluster_size_max", StartCand->getMaxClusterSize() );
          ++scounter;
       }
    }
